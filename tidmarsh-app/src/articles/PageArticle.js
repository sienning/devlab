...
class PageArticle extends Component {
  ...
  chargementAPI = async () => {
    ...
    let temperature =
      "https://chain-api.media.mit.edu/aggregate_data/?timestamp__lt=1602617712&sensor_id=12021&timestamp__gte=1559417712&aggtime=1d";
    ...
    await axios
      .all([rqTemperature, rqHumidity, rqPressure])
      .then(
        axios.spread((...responses) => {
          console.log(
            "rqTemperature",
            responses[0].data.data[responses[0].data.data.length - 1]
          );
          ...
          this.setState({
            isLoading: false,
            temperature: responses[0].data.data,
            humidity: responses[1].data,
            pressure: responses[2].data,
          });
        })
      )
      .catch((error) => {
  ...
  chargementHumidite = async () => {
    ...
    await axios
      .get("https://chain-api.media.mit.edu/scalar_sensors/8")
      .then((response) => {
  ...
  render() {
    const { isLoading, temperature, humidity, pressure } = this.state;
    return (
      <div>
        {isLoading ? (
          <Chargement
